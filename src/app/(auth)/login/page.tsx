...
import { ArrowLeft, Building2, TrendingUp, CheckCircle2, Loader2, Store, Wallet } from "lucide-react";
...
export default function LoginPage() {
  ...
  // Flag: apakah user menekan tombol wallet login
  const pendingWalletAuth = useRef(false);

  const { connect, publicKey, isConnected, isFreighterInstalled, isCheckingInstall } = useStellarWallet();

  // Kalau wallet baru tersambung dan user memang mau login via wallet, langsung auth
  useEffect(() => {
    if (isConnected && publicKey && pendingWalletAuth.current) {
      pendingWalletAuth.current = false;
      authenticateWithWallet(publicKey);
    }
  ...
  const handleWalletLogin = async () => {
    ...
    try {
      if (isConnected && publicKey) {
        // Sudah connected, langsung auth
        await authenticateWithWallet(publicKey);
      } else {
        // Belum connected — set flag lalu panggil connect()
        // useEffect akan handle auth setelah publicKey tersedia
        pendingWalletAuth.current = true;
        await connect();
        // Jika connect() selesai tapi isConnected masih false (user cancel),
        // reset loading
        if (!pendingWalletAuth.current) return; // sudah dihandle useEffect
        // Kalau masih pending berarti connect gagal / ditolak
        pendingWalletAuth.current = false;
  ...
  const authenticateWithWallet = async (walletKey: string) => {
    try {
      const res = await fetch("/api/auth/wallet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ walletAddress: walletKey }),
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Wallet auth gagal");
      ...
      setError(err.message || "Wallet auth gagal");
    } finally {
      setWalletLoading(false);
    }
  };
        ...
        >
          ...
          <div className="flex-1 p-8 sm:p-12 flex flex-col justify-center">
            <div className="max-w-sm mx-auto w-full">
              ...
              {/* Wallet Connect */}
              {isCheckingInstall ? (
                <div className="w-full h-11 flex items-center justify-center gap-2 border border-gray-200 bg-gray-50 rounded-xl text-sm text-gray-400">
                ...
                <button
                  ...
                  className="w-full h-11 flex items-center justify-center gap-2.5 border border-gray-200 bg-white text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {walletLoading ? (
                  ...
                  {walletLoading ? "Menghubungkan..." : "Connect Freighter Wallet"}
                </button>
              ) : (
                <a
                  ...
                  className="w-full h-11 flex items-center justify-center gap-2.5 border border-orange-200 bg-orange-50 text-orange-700 rounded-xl text-sm font-medium hover:bg-orange-100 transition-all"
                >
                  <Wallet className="w-4 h-4" />
                  Install Freighter untuk masuk dengan wallet
                </a>
              )}
