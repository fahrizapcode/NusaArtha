...
import {
  Briefcase, TrendingUp, Store, ChevronRight, PieChart,
  Wallet, Loader2, ExternalLink, RefreshCw,
} from "lucide-react";
...
export default function PortfolioPage() {
  const { isConnected, publicKey, xlmBalance, poolBalances, refreshBalances } = useStellarWallet();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const enriched: EnrichedInvestment[] = await Promise.all(
        invs.map(async (inv) => {
          ...
          const progress = Math.min(
            100,
            Math.round((collected / (inv.pool.totalSupply * inv.pool.pricePerToken)) * 100)
          );
      ...
      const totalValue = enriched.reduce((s, i) => s + i.tokensOwned * i.pool.pricePerToken, 0);
    ...
    <div className="max-w-[1200px] mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Pantau perkembangan aset dan token Anda.</p>
        </div>
      ...
      {isConnected && publicKey && (
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl p-5 text-white">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <p className="text-blue-200 text-xs font-medium uppercase tracking-wide mb-1">Stellar Wallet</p>
              <p className="font-mono text-sm opacity-80">{publicKey.slice(0, 8)}…{publicKey.slice(-6)}</p>
            </div>
            <div className="flex items-end gap-6">
              <div>
                <p className="text-blue-200 text-xs">XLM Balance</p>
                <p className="text-2xl font-bold">{xlmBalance.toFixed(2)} <span className="text-sm font-normal">XLM</span></p>
              </div>
              <div>
                <p className="text-blue-200 text-xs">Token Pools</p>
                <p className="text-2xl font-bold">{poolBalances.length} <span className="text-sm font-normal">aset</span></p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: "Total Investasi", value: stats.total.toString(), icon: Store, color: "text-blue-600 bg-blue-50" },
          {
            label: "Nilai Portfolio",
            value: stats.totalValue > 0 ? `Rp ${(stats.totalValue / 1e6).toFixed(1)}Jt` : "Rp 0",
            icon: Briefcase,
            color: "text-indigo-600 bg-indigo-50",
          },
          { label: "Campaign Aktif", value: stats.activeCampaigns.toString(), icon: PieChart, color: "text-orange-600 bg-orange-50" },
          { label: "Token Pool", value: poolBalances.length.toString(), icon: Wallet, color: "text-green-600 bg-green-50" },
        ].map((card, i) => (
      ...
      <div>
        ...
        {!isConnected && investments.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {investments.map((inv) => {
              const statusCfg = STATUS_LABEL[inv.pool.status] || { label: inv.pool.status, cls: "bg-gray-100 text-gray-600" };
                ...
                <div key={inv.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-900 text-sm">{inv.pool.name}</h3>
                        <p className="text-xs text-gray-500">{inv.pool.brand.name}</p>
                      </div>
                    </div>
                    <span className={cn("text-[10px] font-bold px-2.5 py-1 rounded-md uppercase", statusCfg.cls)}>
                      {statusCfg.label}
                    </span>
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between text-xs text-gray-500 mb-1.5">
                      ...
                      <span className="font-semibold text-gray-900">{inv.progress}%</span>
                    </div>
                    <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full" style={{ width: `${inv.progress}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-4 pt-3 border-t border-gray-50">
                    <div>
                      <p className="text-[10px] font-semibold text-gray-500 uppercase">Token DB</p>
                      <p className="text-sm font-bold text-gray-900">{inv.tokensOwned}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-gray-500 uppercase">Token On-chain</p>
                      <p className={cn("text-sm font-bold", inv.onChainBalance > 0 ? "text-green-600" : "text-gray-400")}>
                        {inv.onChainBalance > 0 ? inv.onChainBalance : "—"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-gray-500 uppercase">Kepemilikan</p>
                      <p className="text-sm font-bold text-gray-900">{inv.ownership}%</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold text-gray-500 uppercase">Nilai Investasi</p>
                  ...
                  <div className="flex items-center gap-2">
                    <Link href={`/investor/dashboard/marketplace/${inv.pool.id}`} className="flex-1">
                      <Button variant="outline" size="sm" className="w-full justify-between border-gray-200 text-gray-700 h-8 text-xs group hover:bg-gray-50">
                        Detail Campaign
      ...
      {isConnected && poolBalances.length > 0 && (
        <div>
          <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
            ...
            <span className="text-xs font-normal text-gray-400">dari {publicKey?.slice(0, 6)}…</span>
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {poolBalances.map((b, i) => (
              <div key={i} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
                <p className="text-xs font-mono text-gray-500 truncate">{b.assetCode}</p>
                <p className="text-lg font-bold text-gray-900 mt-1">{b.balance}</p>
                <p className="text-[10px] text-gray-400 truncate">{b.issuer.slice(0, 8)}…</p>
              </div>
            ))}
          </div>
        </div>
